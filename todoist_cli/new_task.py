...
def preprocess_task_metadata(labels, labels_response, project, projects_response, priority, interactive):
    return (find_label_ids(labels, labels_response, interactive), 
...
def find_label_ids(labels, labels_response, interactive):
    ...
    if interactive and not labels:
        ...
        labels = [all_labels[i].replace('@', '') for i in cutie.select_multiple(all_labels)]
    ...
    if labels:
        ...
        if not cutie.prompt_yes_or_no("Do you want to continue without labels {}".format(', '.join(labels))):
